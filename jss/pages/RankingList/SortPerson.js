...
export default class Person extends PureComponent{
    renderFiexedCol=({item})=>{
        return (
            <View style={{width:getPixel(140),flexDirection:'row'}}>
                <View style={{width:getPixel(70),justifyContent:'center',alignItems:'center',height:getPixel(40),backgroundColor:'#ffffff',}}><Text style={{textAlign:'center'}}>{item.order}</Text></View>
                <View style={{width:getPixel(70),justifyContent:'center',alignItems:'center',height:getPixel(40),backgroundColor:'#ffffff',}}><Text style={{textAlign:'center'}}>{item.name}</Text></View>
            </View>
        )
    ...
    scrollHeaderComponent=()=>{
        return (
            <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', height: getPixel(40), backgroundColor: '#f1f1f1' }}>
                <View style={{ width: getPixel(110), height: getPixel(40), justifyContent: 'center', alignItems: 'center', }}><Text style={{ fontSize: 14, color: '#333333', fontWeight: 'bold' }}>所在部门</Text></View>
    ...
    render(){
        const {sortPersonal}=appState.rankingListData;
        const amount =sortPersonal.reduce(function(s,i){return (s+Number(i.amount))},0);
        const target=sortPersonal.reduce(function(s,i){return (s+Number(i.target))},0);
        const percent=Number(Number(amount)/Number(target)*100).toFixed(2);
        const scrollData=sortPersonal.map(item=>[`${item.department}`,`￥${money(item.target)}`,`￥${money(item.amount)}`,`${item.completeRate}`,`${item.amountRate}`]);
        const scrollTableData=scrollData.reduce(function(s,i){return (s.concat(i))},[])
        return (
            <ScrollView>
            <View style={styles.container}>
                <Bar data={sortPersonal} />
